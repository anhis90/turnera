...
function App() {
  ...
  const [isAdmin, setIsAdmin] = useState(() => {
    try { return localStorage.getItem('isAdmin') === 'true'; } catch (e) { return false; }
  });
  ...
  // If user tries to open admin view but isn't authenticated, redirect to admin login
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex font-sans selection:bg-brand-violet/20">
      ...
      <main className={`flex-1 p-6 md:p-12 pt-28 lg:pt-12 transition-all duration-500 ${isSidebarOpen ? 'lg:ml-72' : 'lg:ml-20'}`}>
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'admin' && !isAdmin && null}
          {view === 'admin-login' && (
            <AdminLogin setIsAdmin={setIsAdmin} setView={setView} />
          )}

        {/* Redirect to admin-login if someone tries to open admin without being admin */}
